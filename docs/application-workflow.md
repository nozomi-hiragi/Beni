...
- 追加情報は、Codex が必要なリソースの識別子を選んだ後で Linear SDK または GraphQL API から取得する。
...
flowchart TD
    ...
    H --> I[Linear SDK または GraphQL API で必要な情報だけを取得]
    I --> F
...
追加情報が必要な場合は、指定された識別子に対応するリソースだけをLinear SDKまたはGraphQL APIから取得し、コンテキストへ加えて再判定します。すべてのLinearイベントを事前収集することや、ワークスペース全体をローカルへ複製することはしません。
...
工程ごとに親イシュー配下のサブイシューをLinear SDKで作成します。作成者がBeniであり、計画段階のBacklogにあるサブイシューだけを、計画修正の対象にできます。すでに実行を開始したものや完了したサブイシューは、計画を直す目的で書き換えません。
